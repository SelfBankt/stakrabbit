...
"Apply" sends a NIP-04 encrypted direct message (`kind: 4`) to the poster's pubkey via `sendApplication()` / `sendEncryptedDM()`, tagged with an `["a", "30402:" + listing.id]` reference so replies can be threaded (see "Referencing a listing from another event" above). There is no reply/inbox concept on the relay side — `handleIncomingDM()` decrypts every kind-4 DM addressed to `currentUser.pubkey` and correlates it to a listing/application locally by that tag.
